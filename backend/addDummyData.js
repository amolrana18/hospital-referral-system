...
async function addDummyData() {
  try {
    console.log('🌱 Adding dummy data to existing hospitals...');
    ...
    for (let i = 0; i < hospitals.length; i++) {
      ...
      const [existingAdmin] = await pool.execute(`
        ...
        WHERE hs.hospital_id = ? AND u.user_role = 'Hospital Admin'
      `, [hospital.hospital_id]);

      if (existingAdmin.length > 0) {
        console.log(`   ⚠️  Hospital already has admin, skipping...`);
        continue;
      }
      ...
      // 5. Create Beds (check if beds already exist)
      const [existingBeds] = await pool.execute('SELECT COUNT(*) as count FROM beds WHERE hospital_id = ?', [hospital.hospital_id]);
      ...
      // 6. Create Ambulance Services (check if ambulances already exist)
      const [existingAmbulances] = await pool.execute('SELECT COUNT(*) as count FROM ambulance_services WHERE hospital_id = ?', [hospital.hospital_id]);
      if (existingAmbulances[0].count === 0) {
    ...
    if (allCredentials.length === 0) {
      console.log('\n⚠️  No new data was added. All hospitals already have staff.');
